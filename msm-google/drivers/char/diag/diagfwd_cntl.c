...
int diag_send_peripheral_buffering_mode(struct diag_buffering_mode_t *params)
{
	...
	pr_err("[diag_redbull][Yuxin][%s]: peripheral=%d, buffering mode=%d, realtime mode=%d\n", __func__, peripheral, params->mode, mode);
		...
		uint8_t diag_id, int peripheral)
{
	...
	if (diag_id && driver->feature[peripheral].pd_buffering) {
		...
		// pr_err("[diag_redbull][Yuxin][%s]: send v2 drain ctrl pkt, peripheral=%d, pkt id=%d, len=%d, version=%d, diag id=%d, stream id=%d\n", __func__, peripheral, ctrl_pkt_v2.pkt_id, ctrl_pkt_v2.len, ctrl_pkt_v2.version, ctrl_pkt_v2.diag_id, ctrl_pkt_v2.stream_id);
	} else {
		...
		// pr_err("[diag_redbull][Yuxin][%s]: send v1 drain ctrl pkt, peripheral=%d\n", __func__, peripheral);
	}
