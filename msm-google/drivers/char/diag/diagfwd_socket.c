...
static void socket_data_ready(struct sock *sk_ptr)
{
	...
	// pr_err("[Yuxin][redbull][%s]diag: wake up socket_read at %llu\n", __func__, t1);
...
static void socket_open_client(struct diag_socket_info *info)
{
	...
	pr_err("[diag_redbull][Yuxin][%s]: client open channel\n", __func__);
}
...
static void socket_open_server(struct diag_socket_info *info)
{
	...
	pr_err("[diag_redbull][Yuxin][%s]: server, socket receive buffer size=%u bytes\n", __func__, size);
}
...
static int diag_socket_read(void *ctxt, unsigned char *buf, int buf_len)
{
	...
	t1 = ktime_to_us(ktime_get_real());
	// pr_err("[Yuxin][redbull][%s]diag: start at %llu us\n", __func__, t1);
	...
	t3 = ktime_to_us(ktime_get_real());
	// pr_err("[Yuxin][redbull][%s]diag: wake up, %llu us\n", __func__, t3-t1);
	...
	// t4 = ktime_to_us(ktime_get_real());
	// pr_err("[Yuxin][redbull][%s]diag: before recvmsg loop, %llu us\n", __func__, t4-t1);
	...
	do {
		...
		if (!info->hdl) {
			...
			"%s closing read thread\n",
				info->name);
			mutex_unlock(&info->socket_info_mutex);
			pr_err("[Yuxin][redbull][%s]diag: %s closing read thread\n", __func__, info->name);
			goto fail;
		}
		err =  info->hdl->ops->ioctl(info->hdl, TIOCINQ,
					(unsigned long)&pkt_len);
		...
		if (pkt_len > bytes_remaining) {
			buf_full = 1;
			mutex_unlock(&info->socket_info_mutex);
			pr_err("[Yuxin][redbull][%s]diag: pkt len=%d, bytes_remaining=%d\n", __func__, pkt_len, bytes_remaining);
			break;
		}
		...
		read_len = kernel_recvmsg(info->hdl, &read_msg, &iov, 1,
					  pkt_len, MSG_DONTWAIT);
		t2 = ktime_to_us(ktime_get_real());
		mutex_unlock(&info->socket_info_mutex);
		if (unlikely(read_len == -ENETRESET)) {
			pr_err("[Yuxin][redbull][%s]diag: socket close channel\n", __func__);
			mutex_lock(channel_mutex);
			diagfwd_channel_read_done(info->fwd_ctxt, buf, 0);
			mutex_unlock(channel_mutex);
			socket_close_channel(info);
		...
		// if (diag_send_nl_message(nl_sk, nlh->nlmsg_pid, kernel_message, strlen(kernel_message)) < 0) {
        // 	pr_err("[Tianyang] socket_send try before recvmsg readlen \n");
    	// }
		/*Ty add end*/

		pr_err("[Yuxin][redbull][%s]diag: recvmsg read_len=%d, total_recd=%d, data_ready=%d, timestamp=%llu us\n", __func__, read_len, total_recd, info->data_ready, t2);
	} while (info->data_ready > 0);

	// t5 = ktime_to_us(ktime_get_real());
	// pr_err("[Yuxin][redbull][%s]diag: after recvmsg loop, %llu us\n", __func__, t5-t1);
	...
	if (total_recd > 0) {
		...
		// t6 = ktime_to_us(ktime_get_real());
		// pr_err("[Yuxin][redbull][%s]diag: ready for data processing, %llu us\n", __func__, t6-t1);

		err = diagfwd_channel_read_done(info->fwd_ctxt, buf,
						total_recd);

		// t7 = ktime_to_us(ktime_get_real());
		// pr_err("[Yuxin][redbull][%s]diag: data processing done, %llu us\n", __func__, t7-t1);
	...
	// t2 = ktime_to_us(ktime_get_real());
	// pr_err("[Yuxin][redbull][%s]diag: end at %llu us\n", __func__, t2-t1);
