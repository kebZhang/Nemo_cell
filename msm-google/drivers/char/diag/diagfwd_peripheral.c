				   ...
				   struct diagfwd_buf_t *buf, int len)
{
	...
	if (hdlc_disabled) {
		...
		pr_err("[Yuxin][redbull][%s]diag: hdlc disabled, write len=%d\n", __func__, write_len);
	} else {
		if (!buf) {
			pr_err("diag: In %s, no match for non encode buffer %pK, peripheral %d, type: %d\n",
				__func__, buf, fwd_info->peripheral,
		...
		pr_err("[Yuxin][redbull][%s]diag: hdlc encoded, write len=%d\n", __func__, write_len);
	}
	mutex_unlock(&fwd_info->data_mutex);
	mutex_unlock(&driver->hdlc_disable_mutex);
	...
	if (write_len > 0) {
		err = diag_mux_write(DIAG_LOCAL_PROC, write_buf, write_len,
				     buf->ctxt);
		if (err) {
			pr_err_ratelimited("diag: In %s, unable to write to mux error: %d, peripheral=%d\n", __func__, err, peripheral);
			goto end_write;
		}
	}
