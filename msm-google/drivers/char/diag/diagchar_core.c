...
static int diag_id_logger = 0;
static int peripheral_logger = 0;
		...
		struct diag_logging_mode_param_t *param, int pid)
{
	int new_mode;
	struct diag_md_session_t *session_info = NULL;
	// pr_err("[diag_redbull][Yuxin][%s]: Start\n", __func__);
	...
	// pr_err("[diag_redbull][Yuxin][%s]: peripheral=%d, req_mod=%d, peripheral_mask=0x%x, device_mask=%d, pd_mask=0x%x\n", __func__, param->peripheral, param->req_mode, param->peripheral_mask, param->device_mask, param->pd_mask);
...
static int diag_switch_logging(struct diag_logging_mode_param_t *param)
{
	...
	// pr_err("[diag_redbull][Yuxin][%s]: Start, peripheral=%d, req_mod=%d, peripheral_mask=0x%x, device_mask=%d, pd_mask=0x%x\n", __func__, param->peripheral, param->req_mode, param->peripheral_mask, param->device_mask, param->pd_mask);
	...
	// pr_err("[diag_redbull][Yuxin][%s]: Before switching, peripheral=%d, req_mod=%d, peripheral_mask=0x%x, device_mask=%d, pd_mask=0x%x\n", __func__, param->peripheral, param->req_mode, param->peripheral_mask, param->device_mask, param->pd_mask);
	...
	pr_err("[diag_redbull][Yuxin][%s]: peripheral %d switch to mode %d, result %d\n", __func__, param->peripheral, param->req_mode, err);
...
static int diag_ioctl_set_buffering_mode(unsigned long ioarg)
{
	...
	// pr_err("[diag_redbull][Yuxin][%s]: peripheral=%d, buffering mode=%d\n", __func__, params.peripheral, params.mode);
...
static int diag_ioctl_peripheral_drain_immediate(unsigned long ioarg)
{
	...
	// pr_err("[diag_redbull][Yuxin][%s]: peripheral=%d\n", __func__, peripheral);
	peripheral_logger = peripheral;
	diag_id_logger = diag_id;
			   ...
			   unsigned int iocmd, unsigned long ioarg)
{
	...
	case DIAG_IOCTL_SWITCH_LOGGING:
		pr_err("[diag_redbull_1120][Yuxin][ioctl]: DIAG_IOCTL_SWITCH_LOGGING\n");
		if (copy_from_user((void *)&mode_param, (void __user *)ioarg,
				   sizeof(mode_param)))
			return -EFAULT;
		// pr_err("[diag_redbull][Yuxin][ioctl]: Copy, peripheral=%d, req_mod=%d, peripheral_mask=0x%x, device_mask=%d, pd_mask=0x%x\n", mode_param.peripheral, mode_param.req_mode, mode_param.peripheral_mask, mode_param.device_mask, mode_param.pd_mask);
...
// int drain_yuxin_thread_func(void *data) {
...
// 		ctrl_pkt_v2.diag_id = diag_id_logger;
// 		ctrl_pkt_v2.stream_id = 1;
// 		err = diagfwd_write(peripheral_logger, TYPE_CNTL, &ctrl_pkt_v2,
// 				sizeof(ctrl_pkt_v2)); // peripheral=0
// 		if (err && err != -ENODEV) {
// 			pr_err("diag: Unable to send drain immediate ctrl packet to peripheral 0, err: %d\n", err);
// 		}
//         msleep(1);
//     }
...
// void drain_yuxin_work_func(struct work_struct *work) {
// 	struct task_struct *drain_yuxin_thread;

//     pr_err("[Yuxin][workqueue] Creating drain thread from workqueue\n");
//     drain_yuxin_thread = kthread_create(drain_yuxin_thread_func, NULL, "drain_yuxin_thread");
//     if (IS_ERR(drain_yuxin_thread)) {
//         pr_err("[Yuxin][drain_thread] Failed to create drain_thread, error: %ld\n", PTR_ERR(drain_yuxin_thread));
//     } else {
//         wake_up_process(drain_yuxin_thread);
//         pr_err("[Yuxin][drain_thread] Drain thread started successfully\n");
//     }
// }
...
{
	int result = -EINVAL;
	// static struct workqueue_struct *drain_yuxin_workqueue;
	// static DECLARE_WORK(drain_yuxin_work, drain_yuxin_work_func);
	...
	case DIAG_IOCTL_GET_REAL_TIME:
		// pr_err("[diag_redbull][Yuxin][ioctl]: DIAG_IOCTL_BUF_DRAIN_START\n");
		// drain_yuxin_workqueue = create_singlethread_workqueue("drain_yuxin_workqueue");
		// if (drain_yuxin_workqueue) {
		// 	queue_work(drain_yuxin_workqueue, &drain_yuxin_work);
		// } else {
		// 	pr_err("[Yuxin][workqueue] Failed to create workqueue\n");
		// }
		result = diag_ioctl_get_real_time(ioarg);
		break;
	case DIAG_IOCTL_PERIPHERAL_BUF_CONFIG:
		// pr_err("[diag_redbull][Yuxin][ioctl]: DIAG_IOCTL_PERIPHERAL_BUF_CONFIG\n");
		result = diag_ioctl_set_buffering_mode(ioarg);
		break;
	case DIAG_IOCTL_PERIPHERAL_BUF_DRAIN:
		// pr_err("[diag_redbull][Yuxin][ioctl]: DIAG_IOCTL_PERIPHERAL_BUF_DRAIN\n");
		result = diag_ioctl_peripheral_drain_immediate(ioarg);
		break;
	// case DIAG_IOCTL_BUF_DRAIN_START:
	// 	pr_err("[diag_redbull][Yuxin][ioctl]: DIAG_IOCTL_BUF_DRAIN_START\n");
	// 	drain_yuxin_workqueue = create_singlethread_workqueue("drain_yuxin_workqueue");
	// 	if (drain_yuxin_workqueue) {
	// 		queue_work(drain_yuxin_workqueue, &drain_yuxin_work);
	...
	// 	}
	// 	break;
	// case DIAG_IOCTL_BUF_DRAIN_END:
	// 	pr_err("[diag_redbull][Yuxin][ioctl]: DIAG_IOCTL_BUF_DRAIN_END\n");
	// 	break;
	}
	return result;
}
			   ...
			   unsigned int iocmd, unsigned long ioarg)
{
	...
	if (iocmd == DIAG_IOCTL_COMMAND_REG) {
		...
		result = diagchar_ioctl_remote(filp, iocmd, ioarg);
			...
			iocmd <= DIAG_IOCTL_PERIPHERAL_BUF_DRAIN) {
		// pr_err("[diag_redbull][Yuxin][ioctl]: diag_ioctl_buffering()\n");
		result = diagchar_ioctl_buffering(filp, iocmd, ioarg);
	} else if (iocmd >= DIAG_IOCTL_REGISTER_CALLBACK &&
			iocmd <= DIAG_IOCTL_HDLC_TOGGLE) {
		result = diagchar_ioctl_misc(filp, iocmd, ioarg);
			  ...
			  loff_t *ppos)
{
	...
	// pr_err("[Yuxin][redbull][%s]diag: wake up read at %llu us\n", __func__, t1);
...
/*Ty add*/
int diag_send_direct(char *message)
{
...
int diag_send_nl_message(char *message, int pid, int message_len) 
{
    struct sk_buff *skb_out;
    struct nlmsghdr *nlh_out;
	// char *message = "FromKernel";
	int i=0;
	int res=0;
	

	pr_err("[Tianyang] [diag_send_nl_msg] enter func, pid = %d, msg_len = %d\n", pid, message_len);

	//message[message_len]='\0';
	//message_len+=1;
	//pr_err("[Tianyang] [diag_send_nl_msg]\n");
	for(i=0;i<message_len;i++)
	{
		pr_err("[Tianyang] [diag_send_nl_msg] %c", message[i]);
	}
    ...
    if (!skb_out) {
		pr_err("[Tianyang] [diag_send_nl_msg] fail in set sk_buff \n");
        return -ENOMEM;
    }
	...
	pr_err("[Tianyang] [diag_send_nl_msg] init header \n");

    // Set the destination PID and copy the message data
    // NETLINK_CB(skb_out).dst_group = 0; /* unicast */
	// NETLINK_CB(skb_out).portid = 0; /* unicast */
	...
	if(res<0)
	{
		pr_err("[Tianyang] [diag_send_nl_msg] fail in send op final \n");
		return 0;
	}
	pr_err("[Tianyang] [diag_send_nl_msg] success final \n");

    return 0;
}

int diag_send_nl_message_in(int pid, int message_len) 
{
    struct sk_buff *skb_out;
    struct nlmsghdr *nlh_out;
	char *message = "FromKernel";
    ...
    // Initialize the netlink message header
    // nlh_out = nlmsg_put(skb_out, 0, 0, NLMSG_DONE, message_len, 0);
	nlh_out = nlmsg_put(skb_out, 0, 0, NLMSG_DONE, 11, 0);
    if (!nlh_out) {
        nlmsg_free(skb_out);
        return -ENOMEM;
    }
    ...
    // NETLINK_CB(skb_out).dst_group = 0; /* unicast */
	// NETLINK_CB(skb_out).portid = 0; /* unicast */
    memcpy(NLMSG_DATA(nlh_out), message, 11);
	//strncpy(nlmsg_data(nlh), message, 11);
...
void diag_nl_recv_msg(struct sk_buff *__skb)
{
	//char *kernel_message = "from kernel";
    struct sk_buff *skb;
    char str[MSG_SIZE] = {0};
	//char *msg_send = "FromKernel"; 
	...
	msg_send = kmalloc(MSG_SIZE, GFP_KERNEL);
	memcpy(msg_send, "Kernel_1205_1", MSG_SIZE);
	//strncpy(msg_send, "Kernel_1205_1", MSG_SIZE);
	msg_send[MSG_SIZE - 1] = '\0';
    ...
    printk(KERN_INFO "[Tianyang] receive message (pid:%d):%s\n", nlh->nlmsg_pid, str);
	pr_err("[Tianyang] receive msg \n");

	//will stuck here
	pr_err("[Tianyang] before try to send \n");

	
	msg_len = strlen(msg_send);
	pr_err("[Tianyang] msg_len = %d \n", msg_len);
	res = diag_send_nl_message(msg_send, nlh->nlmsg_pid, MSG_SIZE);
	//res = diag_send_nl_message_in(nlh->nlmsg_pid, msg_len);
	pr_err("[Tianyang] try to send success \n");
	kfree(msg_send);

    // if (diag_send_nl_message(nl_sk, nlh->nlmsg_pid, str, strlen(str)) < 0) {
    //     pr_err("[Tianyang] try to send fail \n");
    // }
	// else
	// {
	// 	pr_err("[Tianyang] try to send success \n");
...
int diag_netlink_init(void)
{
    ...
    if (!nl_sock) {
        printk(KERN_ERR "[Tianyang] diag: Failed to create netlink socket\n");
        return -ENOMEM;
    }
	pr_err("[Tianyang] diag_netlink_init netlink socket initialized 18\n");
    printk(KERN_INFO "[Tianyang] diag: Netlink socket initialized\n");
    return 0;
}

int diag_netlink_exit(void)
{
	netlink_kernel_release(nl_sock);
    printk(KERN_INFO "[Tianyang] Netlink socket released\n");
	pr_err("[Tianyang] netlink socket exit\n");
	return 0;
}
/*Ty add end*/
...
static int __init diagchar_init(void)
{
	...
	pr_err("[diag_redbull][Yuxin][%s]: diagfwd initializing ..\n", __func__);
	ret = 0;
	...
	/*Ty add*/
	ret = diag_netlink_init();
	if(ret)
	{
		pr_err("[Tianyang] diag_netlink_init failed\n");
	}
	else
	{
		pr_err("[Tianyang] diag_netlink_init succeed\n");
	}
	/*Ty add end*/
	...
	pr_err("[diag_redbull][Yuxin][%s]: diagchar initialized now\n", __func__);
	#ifdef CONFIG_DIAGFWD_BRIDGE_CODE
